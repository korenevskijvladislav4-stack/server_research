...
export const slotSelectorService = {
  async getSelectorsByCasino(casinoId: number): Promise<SlotSelector[]> {
    const rows = await prisma.selectors.findMany({
  ...
  async createSelector(data: CreateSlotSelectorDto): Promise<SlotSelector> {
    const row = await prisma.selectors.create({
      data: {
  ...
  async updateSelector(id: number, data: UpdateSlotSelectorDto): Promise<SlotSelector | null> {
    const updateData: Prisma.selectorsUpdateInput = {
      updated_at: new Date(),
    };
    ...
    if (Object.keys(updateData).length === 1) {
      const existing = await prisma.selectors.findUnique({ where: { id } });
    ...
    const row = await prisma.selectors.update({
      where: { id },
      data: updateData,
    });
  ...
  async deleteSelector(id: number): Promise<void> {
    await prisma.selectors.deleteMany({
      where: { id },
    });
  },
};
